    ...
    Q: CustomMsg,
{
    ...
    fn approval(
        ...
        if token.owner == spender {
            let approval = cw721_updatable::Approval {
        ...
        let filtered: Vec<_> = token
            ...
            .map(|a| cw721_updatable::Approval {
...
fn parse_approval(item: StdResult<(Addr, Expiration)>) -> StdResult<cw721_updatable::Approval> {
    item.map(|(spender, expires)| cw721_updatable::Approval {
...
fn humanize_approval(approval: &Approval) -> cw721_updatable::Approval {
    cw721_updatable::Approval {
